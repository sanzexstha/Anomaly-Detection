...
# from flop_counter import EinsumWrapper
...
class DozerAttention(nn.Module):
    def __init__(self, local_window, stride, rand_rate, vary_len, pred_len, mask_flag=False, scale=None, attention_dropout=0.1, output_attention=True):
    ...
    def forward(self, queries, keys, values, attn_mask):
        ...
        for i in range(L_Q):
            ...
            # seleted_keys_idxs = torch.arange(e)
            # seleted_keys_idxs = torch.randint(0, 10, (x,))
            print(len(seleted_keys_idxs))

            # q: (B, 1, H, E)   – single‑position query at index i

            # scores[:, :, i:i + 1, seleted_keys_idxs] = einsum_op(
            #   queries[:, i:i + 1, :, :],
...
for i in range(len(s_l)):
    B, L_Q, L_K, H, D = 32, s_l[i],s_l[i], 8, 64
    ...
    # Model
    # model1 = FullAttention(mask_flag=False, scale=None, attention_dropout=0.1, output_attention=True)

    # model2 = DozerAttention(local_window=5, stride=7, rand_rate=0.1, vary_len=2, pred_len=10)

    model = DozerAttention(local_window=5, stride=20, rand_rate=0.1, vary_len=2, pred_len=10)
    flop_counter1 = FlopCountAnalysis(model, (queries, keys, values, attn_mask)).by_operator()
    if i == 0:
        prev = flop_counter1["einsum"]

    print("Scale factor", flop_counter1["einsum"]/ prev)
    ...
    # flop_counter1 = FlopCountAnalysis(model, (queries, keys, values, attn_mask))
    ...
    # flops_full = flop_counter1.by_operator()
    # flops_Dozer = flop_counter2.by_operator()
    # print("Scale factor", flops_Dozer1/ flops1['einsum'])

    # print("dozer", flop_counter2.by_operator())

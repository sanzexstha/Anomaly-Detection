...
class Solver(object):
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            # print(
            #     "Epoch: {0}, Steps: {1} | Vali Loss: {3:.7f} ".format(
            #         epoch + 1, train_steps, vali_loss))
            print(f"Epoch: {epoch + 1}, Steps: {train_steps} | Vali Loss: {vali_loss:.7f}")
    ...
    def test(self):
        ...
        for i, (input_data, labels) in enumerate(self.thre_loader):
            input = input_data.float().to(self.device)
            output = self.model(input)   #output, series, prior, _ = self.model(input)
